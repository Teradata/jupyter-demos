...
for (let i = 0; i < files.length; i++) {
    ...
    test(`test ${i}: ${name}`, async ({page},testInfo) => {
        ...
        const pyKernel = await page.locator('span[class="f1235lqo"]').getByText('Python 3 (ipykernel) | Idle');
        const sqlKernel = await page.locator('span[class="f1235lqo"]').getByText('Teradata SQL | Idle');
        let strKernelType = '';
        if (await pyKernel.isVisible()) {
            console.log('Python Notebook');
            strKernelType = "Python 3 (ipykernel) ";
        } 
        else if (await sqlKernel.isVisible()) {
        ...
        for (let i = 0; i < dm; i++) {
            ...
            await page.locator('span[class="f1235lqo"] >> text="'+strKernelType+'| Idle"').waitFor({timeout: 60000});      
